...
# GROUPING
      # Change line color by sex --> Stroke histogram
      ggplot(mifem, aes(x = age)) + geom_histogram(aes( fill = stroke), color = "black", position = "identity", bins = 30, alpha = 0.4) + 
        ggtitle("Age histogram as function of stroke and outcome") +
        theme_economist() +  xlab("age") + ylab("count") +
        theme(axis.title.y = element_text(margin = margin(t = 0, r = 10, b = 0, l = 0), face = "bold"),
              axis.title.x = element_text(face = "bold"), legend.position = "right",
              title = element_text(margin = margin(t = 0, r = 0, b = 10, l = 0))) +
      scale_fill_discrete(name = "Stroke", labels = c("Yes", "No", "Not known"))
